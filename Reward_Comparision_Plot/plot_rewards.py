...
if __name__=="__main__":
    ...
    domains_to_methods = {"waterworld": ['adrl','adrl'], "mountaincar": ['adrl','adrl'],"cartpole":['qphi','adrl'], "lunar":['qphi','adrl'] }

    # domains_to_methods = {"waterworld": ['adrl','adrl'], "mountaincar": ['adrl','adrl'],"cartpole":['qphi','adrl'], "lunar":['adrl','adrl'] }

    method_to_names = {"adrl":"CA+RL ", "qphi":"Q_learning+$\phi$"}
    
    dirpath = "./acc_rewards/"
    interval_episodes = 10 # need to modify?
    colors = {'adrl': ['tab:green','tab:green'], 'qphi': ['tab:red','tab:red']}
    ...
    for i,domain_name in enumerate(domain_names):
        ...
        # num_ticks = 6
        # y_ticks = np.linspace(min(q_mean), max(a_mean), num_ticks)

        # y_ticks = [0.0,50.0,100.0,150.0,200.0]
        y_ticks = np.arange(round(min(min(a_mean),min(q_mean))), round(max(max(a_mean),max(q_mean))) + 50, 50)
    ...
    fig.text(0.06,0.48, "       Cumulative Reward", va = "center",rotation = "vertical")
